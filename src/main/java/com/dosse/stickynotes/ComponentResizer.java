...
public class ComponentResizer extends MouseAdapter {
	private final static Dimension MINIMUM_SIZE = new Dimension(10, 10),
								   MAXIMUM_SIZE = new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE);
	private static Map<Integer, Integer> cursors = new HashMap<>();
	protected static final int NORTH = 1, WEST = 2, SOUTH = 4, EAST = 8;
	...
	private boolean resizing, autoscrolls;
	
	private Insets dragInsets;
	private Dimension snapSize, minimumSize = MINIMUM_SIZE, maximumSize = MAXIMUM_SIZE;
	private Cursor sourceCursor;
	private Rectangle bounds;
	private Point pressed;
	
	/**
	 * Convenience contructor. All borders are resizable in increments of a single pixel.
	 * Components must be registered separately.
	 */
	public ComponentResizer() { this(new Insets(5, 5, 5, 5), new Dimension(1, 1)); }
	
	/**
	 * Convenience contructor.
	 * All borders are resizable in increments of a single pixel.
	 * Components can be registered when the class is created or they can be registered separately afterwards.
	 *
	 * @param components
	 ...
	 */
	public void setMaximumSize(Dimension maximumSize) { this.maximumSize = maximumSize; }
	 ...
	 */
	public Dimension getSnapSize() { return snapSize; }
	 ...
	 *            vertical snap size.
	 */
	public void setSnapSize(Dimension snapSize) { this.snapSize = snapSize; }
	...
	public void mouseMoved(MouseEvent e) {
		...
		if (direction == 0) source.setCursor(sourceCursor);
	...
	public void mouseEntered(MouseEvent e) {
		if (!resizing) sourceCursor = e.getComponent().getCursor();
	}
	
	@Override
	public void mouseExited(MouseEvent e) {
		if (!resizing) e.getComponent().setCursor(sourceCursor);
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		// Setup for resizing. All future dragging calculations are done based on the original bounds of the component and mouse pressed location.
		resizing = true;
		Component source = e.getComponent();
		...
		// Making sure autoscrolls is false will allow for smoother resizing of components
	...
	public void mouseReleased(MouseEvent e) {
		...
		resizing = false;
		if (source instanceof JComponent) ((JComponent) source).setAutoscrolls(autoscrolls);
	}
	...
	private Dimension getBoundingSize(Component source) {
		...
		} else return source.getParent().getSize();
	}
}
