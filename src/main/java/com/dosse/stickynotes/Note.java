...
public class Note extends JDialog {
	 ...
	 */
	private static String getLocString(String s) { return locBundle.getString(s); }
	
	// <editor-fold defaultstate="collapsed" desc="Color schemes">
	/**
	 * SCHEME FORMAT: {external color, line border color, bar color, buttons color,
	 * internal color, text color, selection background color, selected text color}
	 */
	private static final Color[] YELLOW_SCHEME = new Color[] { new Color(248, 248, 182), new Color(232, 232, 170),
			new Color(248, 248, 182), new Color(105, 105, 77), new Color(253, 253, 202), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] ORANGE_SCHEME = new Color[] { new Color(248, 205, 161), new Color(230, 189, 149),
			new Color(248, 205, 161), new Color(102, 84, 66), new Color(255, 215, 173), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] BLUE_SCHEME = new Color[] { new Color(201, 236, 248), new Color(188, 221, 232),
			new Color(201, 236, 248), new Color(78, 92, 96), new Color(217, 243, 251), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] GREEN_SCHEME = new Color[] { new Color(197, 247, 193), new Color(183, 230, 179),
			new Color(197, 247, 193), new Color(79, 96, 77), new Color(209, 254, 203), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] PINK_SCHEME = new Color[] { new Color(241, 195, 241), new Color(230, 186, 230),
			new Color(241, 195, 241), new Color(99, 80, 99), new Color(246, 211, 246), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] PURPLE_SCHEME = new Color[] { new Color(212, 205, 243), new Color(200, 194, 230),
			new Color(212, 205, 243), new Color(89, 86, 102), new Color(221, 217, 254), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	private static final Color[] RED_SCHEME = new Color[] { new Color(247, 197, 193), new Color(230, 183, 179),
			new Color(247, 197, 193), new Color(96, 79, 77), new Color(254, 209, 203), new Color(0, 0, 0),
			new Color(136, 195, 255), new Color(0, 0, 0) };
	 ...
	 *
	 * A note is initialized empty, with a yellow background and at current mouse
	 * coordinates.
	 */
	public Note() {
		...
		addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				//Save the current state and terminate the app
				Main.saveState();
				System.exit(0);
			}
		});
		...
		text = new JTextArea() {
			...
			//Configures the textarea to resize properly horizontaly (workaround for swing bug)
		...
		doc.addUndoableEditListener(new UndoableEditListener() {
			@Override public void undoableEditHappened(UndoableEditEvent e) { undo.addEdit(e.getEdit()); }
		});
		...
		cr.setMinimumSize(new Dimension((int) (160 * Main.SCALE), (int) (90 * Main.SCALE))); // min size is 160x90
																								// @80dpi
		...
		newNote.addActionListener(new ActionListener() {
			@Override public void actionPerformed(ActionEvent evt) { Main.newNote(); }
		});
		...
		deleteNote.addActionListener(new ActionListener() {
			@Override public void actionPerformed(ActionEvent evt) { Main.delete(Note.this); }
		});
		
		InputHandler ih = new InputHandler();
		text.addMouseListener(ih);
		text.addMouseWheelListener(ih);
		text.addKeyListener(ih);
		
		// right click on top bar
		wrapper2.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent evt) {
				if (evt.isPopupTrigger() || evt.getButton() == MouseEvent.BUTTON3) colorMenu.show(evt.getComponent(), evt.getX(), evt.getY()); // show color selection menu
			}
		});
			...
			public void onColorSelected(Color c) {
				...
				if (hsb[2] < 0.55f) {
					...
					Color internal = new Color(Color.HSBtoRGB(hsb[0], hsb[1], hsb[2])),
							external = new Color(Color.HSBtoRGB(hsb[0], hsb[1], hsb[2] + 0.1f)), bar = external,
					...
					Color internal = new Color(Color.HSBtoRGB(hsb[0], hsb[1], hsb[2])),
							external = new Color(Color.HSBtoRGB(hsb[0], hsb[1], hsb[2] - 0.04f)), bar = external,
		...
		wrapper2Layout.setHorizontalGroup(wrapper2Layout.createParallelGroup(GroupLayout.Alignment.LEADING)
					  .addGroup(wrapper2Layout.createSequentialGroup().addComponent(newNote)
					  .addPreferredGap(LayoutStyle.ComponentPlacement.RELATED, 134, Short.MAX_VALUE)
					  .addComponent(deleteNote))
					  .addComponent(jScrollPane1, GroupLayout.PREFERRED_SIZE, 0, Short.MAX_VALUE));
		wrapper2Layout.setVerticalGroup(wrapper2Layout.createParallelGroup(GroupLayout.Alignment.LEADING)
					  .addGroup(GroupLayout.Alignment.TRAILING, wrapper2Layout.createSequentialGroup().addGap(0, 0, 0)
					  .addGroup(wrapper2Layout.createParallelGroup(GroupLayout.Alignment.LEADING)
					  .addComponent(deleteNote, GroupLayout.PREFERRED_SIZE, (int) (19 * Main.SCALE * 0.85f), GroupLayout.PREFERRED_SIZE)
					  .addComponent(newNote, GroupLayout.PREFERRED_SIZE, (int) (19 * Main.SCALE * 0.85f), GroupLayout.PREFERRED_SIZE))
		...
		wrapper1Layout.setHorizontalGroup(wrapper1Layout.createParallelGroup(GroupLayout.Alignment.LEADING).addGroup(GroupLayout.Alignment.TRAILING, wrapper1Layout.createSequentialGroup()
					  .addGap((int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f))
					  .addComponent(wrapper2, GroupLayout.DEFAULT_SIZE, 217, Short.MAX_VALUE)
					  .addGap((int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f))));
		wrapper1Layout.setVerticalGroup(wrapper1Layout.createParallelGroup(GroupLayout.Alignment.LEADING)
					  .addGroup(wrapper1Layout.createSequentialGroup()
					  .addGap((int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f))
					  .addComponent(wrapper2, GroupLayout.DEFAULT_SIZE, 203, Short.MAX_VALUE)
					  .addGap((int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f), (int) (6 * Main.SCALE * 0.85f))));
	...
	@Override public void setLocation(int x, int y) { setBounds(x, y, getWidth(), getHeight()); }
	/** setLocation method is overridden to force the note to stay on the screen */
	@Override public void setLocation(Point p) { setLocation(p.x, p.y); }
	
	/** setBounds method is overridden to force the note to stay on the screen */
	@Override
	public void setBounds(int x, int y, int width, int height) {
		// for some odd fucking reason, this can happen on some versions of java
		if (preferredLocation == null) preferredLocation = new Point(0, 0);
		preferredLocation.x = x;
		preferredLocation.y = y;

		Dimension s = Main.getExtendedScreenResolution();
		if (x + 60 * Main.SCALE > s.width) x = (int) (s.width - 60 * Main.SCALE);
		if (y + 60 * Main.SCALE > s.height) y = (int) (s.height - 60 * Main.SCALE);
		super.setBounds(x, y, width, height);
	}
	
	/** setBounds method is overridden to force the note to stay on the screen */
	@Override public void setBounds(Rectangle r) { setBounds(r.x, r.y, r.width, r.height); }
	...
	/** @return the last user-set location of the note */
	public Point getPreferredLocation() { return preferredLocation; }
	public float getTextScale() { return textScale; }
	
	/** @param scale as float 0.2-4.0 */
	...
	/** "Container" class for condensing input handlers */
	public class InputHandler extends MouseAdapter implements KeyListener {
		...
		public void mouseClicked(MouseEvent evt) {
			if (evt.isPopupTrigger() || evt.getButton() == MouseEvent.BUTTON3) {
				//If there is no text selected, disable cut, copy and delete
				...
				paste.setEnabled(getClipboard() != null); // paste is only enabled if there's something on the clipboard
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			if (e.isControlDown() || e.isMetaDown()) { // scroll wheel with ctrl pressed (isMetaDown is for apple faggots)
				if (e.getWheelRotation() < 0) setTextScale(textScale + 0.1f);
				else if (e.getWheelRotation() > 0) setTextScale(textScale - 0.1f);
			} else jScrollPane1.getMouseWheelListeners()[0].mouseWheelMoved(e); // scroll wheel without ctrl pressed simply scrolls
		}
		
		@Override
		public void keyPressed(KeyEvent e) {
			if (e.isControlDown() || e.isMetaDown()) switch (e.getKeyCode()) {
