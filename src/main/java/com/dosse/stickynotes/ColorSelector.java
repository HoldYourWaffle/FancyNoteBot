...
public abstract class ColorSelector extends JPanel {
	private static final long serialVersionUID = Main.calcVersionId(ColorSelector.class);
	...
	private static final Border NORMAL_BORDER = new LineBorder(new Color(0, 0, 0, 0), (int) (2 * Main.SCALE)),
								SELECTED_BORDER = new LineBorder(SELECTED_COLOR, (int) (2 * Main.SCALE));
	...
	public ColorSelector(Color[][] colorSchemes) {
		...
		for (final Color[] c : colorSchemes) {
			...
			l.addMouseListener(new MouseAdapter() {
				@Override public void mouseEntered(MouseEvent e) { l.setBorder(SELECTED_BORDER); }
				@Override public void mouseExited(MouseEvent e) { l.setBorder(NORMAL_BORDER); }
		...
		// this brutal workaround removes the selected border when the color menu isn't closed properly
		addHierarchyListener(new HierarchyListener() {
			@Override
			public void hierarchyChanged(HierarchyEvent e) {
				for (JLabel b : buttons) b.setBorder(NORMAL_BORDER);
			}
		});
	}
